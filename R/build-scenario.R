...
load_scenario <- function(scenario, params = fallRunDSM::r_to_r_baseline_params,
  ...
  starting_habitat <- case_when(1 %in% action_numbers & 
                                ...
                                1 %in% action_numbers & 
                                  starting_hydrology == "eff" ~ "r_to_r_eff_baseline", 
                                2 %in% action_numbers & 
...
apply_habitat_actions <- function(scenario, params, starting_habitat, starting_hydrology, species) {
  ...
  if(32 %in% scenario$action) {
    updated_habitat$spawning_habitat = updated_habitat$spawning_habitat * 13 
    ...
    updated_habitat$floodplain_habitat = updated_habitat$floodplain_habitat * 5
...
apply_hydrology_actions <- function(scenario, params, starting_hydrology, species) {
  is_calsim_output_version <- ifelse(starting_hydrology %in% c("eff", "LTO_12a_eff_dy"), FALSE, TRUE)
  if(is_calsim_output_version) {
    ...
    san_joaquin_flows <- eval(parse(text = paste0("DSMflow::san_joaquin_flows$eff")))
  }
